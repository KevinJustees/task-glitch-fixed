...
// Stable sorting: ROI → Priority → Title
...
export function computeFunnel(tasks: ReadonlyArray<Task>): FunnelCounts {
  ...
  const conversionTodoToInProgress = total
    ? (inProgress + done) / total
    : 0;

  const conversionInProgressToDone = inProgress
    ? done / inProgress
    : 0;
...
export function computeVelocityByPriority(
  ...
  (Object.keys(groups) as Task["priority"][]).forEach((key) => {
    const arr = groups[key].slice().sort((a, b) => a - b);
    const avg = arr.length
      ? arr.reduce((s, v) => s + v, 0) / arr.length
      : 0;
    const mid = arr.length ? arr[Math.floor(arr.length / 2)] : 0;

    stats[key] = { avgDays: avg, medianDays: mid };
  });
